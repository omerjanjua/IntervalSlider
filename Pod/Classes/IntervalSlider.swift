...
public class IntervalSliderSource {
    ...
    /**
     ...
     - parameter shield: shield to be displayed at the top of the appearanceValue
     */
    public init(validValue: Float, appearanceValue: Float, shield: UIButton) {
...
public class IntervalSlider: UIView {
    ...
    private func build(options: [IntervalSliderOption]?) {
        ...
        for source in self.sources {
            if let shield = source.shield {
                shield.frame = CGRectMake(0, 0, shield.bounds.size.width, shield.bounds.size.height)
                self.shields.append(shield)
                self.addSubview(shield)
            }
            if self.isAddMark {
                let mark = UIView(frame: CGRectMake(0, 0, 20, 20))
                mark.layer.cornerRadius = mark.frame.size.width/2
                mark.clipsToBounds = true
                mark.backgroundColor = UIColor.lightGrayColor()
                self.marks.append(mark)
                self.insertSubview(mark, aboveSubview:self.slider)
            }
        }
    ...
    private func layout(){
        ...
        for source in self.sources {
            ...
            shield.frame = CGRectMake(0,
                                      self.slider.frame.origin.y - (shield.bounds.size.height + self.labelBottomPadding),
                                      shield.bounds.size.width,
                                      shield.bounds.size.height)
            shield.center = CGPoint(x: center.x, y: shield.center.y)
            if self.isAddMark {
